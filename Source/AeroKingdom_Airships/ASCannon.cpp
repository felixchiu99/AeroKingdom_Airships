...
void AASCannon::Fire()
{
	if (bIsFiring && !bOnCooldown) {
		...
		if (FireSound != nullptr)
		{
			...
			//GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Volume " + FString::SanitizeFloat(fSFXVolume)));
			//UGameplayStatics::PlaySoundAtLocation(this, FireSound, FirePoint->GetComponentTransform().GetLocation(), fSFXVolume,1.f,0.0f, AudioSetting);
			GetGameInstance<UGIAirshipGameInstance>()->SSPlaySFXAtLocation(this, FireSound, FirePoint->GetComponentTransform().GetLocation(), FRotator(0,0,0), 1.f, 1.f, 0.0f, AudioSetting);
		}
...
void AASCannon::StartFire()
{
	bIsFiring = true;
	Fire();
}

void AASCannon::StopFire()
{
	bIsFiring = false;
	FireTimer.Invalidate();
}

void AASCannon::ResetCoolDown()
{
	bOnCooldown = false;
	if (bIsFiring)
		Fire();
